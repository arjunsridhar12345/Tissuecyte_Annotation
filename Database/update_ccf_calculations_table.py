...
def update_ccf_and_calculation_tables(df_insertion_channel_info:pd.DataFrame, df_insertion_hit_rate:pd.DataFrame, df_min_displacement_vector:pd.DataFrame,
    ...
    for index, row in df_insertion_channel_info.iterrows():
        if pd.isna(row.Channel_0_AP):
            ...
            try:
                ...
                ccf_coords = df_insertion_channel_info.iloc[index, 9:].to_numpy()
                ...
                for area in area_center_of_mass:
                    ...
                    displacement, min_distance = min_distance_to_ccf_point(ccf_coords, area_center_of_mass[area])
    ...
    df_min_distance.to_sql('min_distance_by_insertion', con=ENGINE, schema=None, if_exists='replace')
    df_min_displacement_vector.to_sql('closest_point_by_insertion', con=ENGINE, schema=None, if_exists='replace')
...
if __name__ == '__main__':
    ...
    df_insertion_channel_info = pd.read_sql_table('insertion_channel_info', con=ENGINE)
    df_insertion_hit_rate = pd.read_sql_table('hit_rate_by_insertion', con=ENGINE)
    df_min_displacement_vector = pd.read_sql_table('closest_point_by_insertion', con=ENGINE)
    df_min_distance = pd.read_sql_table('min_distance_by_insertion', con=ENGINE)
